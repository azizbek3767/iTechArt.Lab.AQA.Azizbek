namespace SeleniumAdvancedPartTwo.Tests
{
    internal class HoversPageTests : BaseTest
    {
        ...
        public void Check_Hovers_And_Links()
        {
            ...
            string path = "http://admin:admin@the-internet.herokuapp.com/users/";
            HoversPage.HoverFirstUserItem();
            Assert.That(HoversPage.FirstUserName, Is.EqualTo("user1"));
            Assert.That(HoversPage.FirstUserLink.GetAttribute("href"), Is.EqualTo(path+"1"));
            HoversPage.HoverSecondUserItem();
            Assert.That(HoversPage.SecondUserName, Is.EqualTo("user2"));
            Assert.That(HoversPage.SecondUserLink.GetAttribute("href"), Is.EqualTo(path + "2"));
            HoversPage.HoverThirdUserItem();
            Assert.That(HoversPage.ThirdUserName, Is.EqualTo("user3"));
            Assert.That(HoversPage.ThirdUserLink.GetAttribute("href"), Is.EqualTo(path + "3"));
            ...
            Assert.That(WebDriver.Url, Is.EqualTo(path + "1"));
            HoversPage.GoBack();

            HoversPage.HoverSecondUserItem();
            HoversPage.ClickSecondUserButton();
            Assert.That(WebDriver.Url, Is.EqualTo(path + "2"));
            HoversPage.GoBack();
            
            HoversPage.HoverThirdUserItem();
            HoversPage.ClickThirdUserButton();
            Assert.That(WebDriver.Url, Is.EqualTo(path + "3"));
